...
async def add_income(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Динамическая подсказка на основе текущего состояния
    if wallets['gold_reserve'] == 0:
        hint = "💡 *Совет:* Этот доход запустит правило 10% - первые накопления!"
    else:
        hint = f"💡 Ваш золотой запас: {wallets['gold_reserve']:,.0f} руб. (+10% от этого дохода)"
...
async def add_expense(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Чистое вавилонское добавление расхода."""
    user_id = update.message.from_user.id
